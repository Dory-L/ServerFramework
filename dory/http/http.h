...
template<class MapType, class T>
    bool checkGetAs(const MapType& m, const std::string& key, T& val, const T& def = T()) {
        ...
        try {
            boost::lexical_cast<T>(it->second);
            return true;
        } catch (...) {
            val = def;
        }
...
public:
     ...
     * @param val 返回值
     * @param def 默认值
     * @return true 如果存在且转换成功返回true，否则失败val=def
     * @return false 
     */
    template<class T>
...
public:
    ...
    uint8_t getVersion() const { return m_version; }
    const std::string& getBody() const { return m_reason; }
    const MapType& getHeaders() const { return m_headers; }
...
std::ostream& operator<<(std::ostream& os, const HttpResponse& rsq);
