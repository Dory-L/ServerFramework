...
int32_t FunctionServlet::handle(dory::http::HttpRequest::ptr request
...
int32_t ServletDispatch::handle(dory::http::HttpRequest::ptr request
                        , dory::http::HttpResponse::ptr response
                        , dory::http::HttpSession::ptr session)  {
...
Servlet::ptr ServletDispatch::getMatchedServlet(const std::string& uri) {
    ...
    for (auto it = m_globs.begin();
            it != m_globs.end(); ++it) {
        if (!fnmatch(it->first.c_str(), uri.c_str(), 0)) { //模糊匹配
            return it->second;
        }
    }
    return m_default;
...
int32_t NotFoundServlet::handle(dory::http::HttpRequest::ptr request
                , dory::http::HttpResponse::ptr response
                , dory::http::HttpSession::ptr session) {
    static const std::string& RSP_BODY = "<html>"
            ...
            "<hr><center>dory/1.0.0</center>"
            "</body>"
            "</html>";

    response->setStatus(dory::http::HttpStatus::NOT_FOUND);
    response->setHeader("Server", "dory/1.0.0");
