...
dory::Logger::ptr g_logger = DORY_LOG_NAME("system");
...
HttpConnection::~HttpConnection() {
    DORY_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if (client_parser.chunked) {
        ...
        do {
            ...
            DORY_LOG_INFO(g_logger) << "conntent_len=" << client_parser.content_len;
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    ...
    if (!ptr) {
        ...
        if (!addr) {
            DORY_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if (!sock) {
            DORY_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if (!sock->connect(addr)) {
            DORY_LOG_ERROR(g_logger) << "sock conect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if (!ptr->isConnected()
            || (ptr->m_createTime + pool->m_maxAliveTime) >= dory::GetCurrentMS()
...
HttpResult::ptr HttpConnectionPool::doRequest(HttpRequest::ptr req
    ...
    if (!conn) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::POOL_GET_CONNECTION
                , nullptr, "pool host:" + m_host + std::to_string(m_port));
    }
